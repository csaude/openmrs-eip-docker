...
if [ -f "$INSTALL_FINISHED_REPORT_FILE" ]; then
	...
	chmod +x $SCRIPTS_DIR/*.sh
        ...
        logToScreenAndFile "Copying $EIP_PACKAGE_RELEASE_FILE_NAME to $HOME_DIR/openmrs-eip-app-sender.jar" $LOG_FILE
        cp "$CURRENT_RELEASES_PACKAGES_DIR/$EIP_PACKAGE_RELEASE_FILE_NAME" "$HOME_DIR/openmrs-eip-app-sender.jar"
        
        logToScreenAndFile "Copying $EPTSSYNC_PACKAGE_RELEASE_FILE_NAME to $EPTSSYNC_HOME_DIR/eptssync-api-1.0-SNAPSHOT.jar" $LOG_FILE
