...
for FILE in *.sh; do
        if [ -f "$FILE"_installed ]; then
                echo "THE SCRIPT $FILE WAS ALREDY RUN" | tee $AFTER_UPGRADE_SCRIPTS_HOME/install.log
        else
                echo "RUNNING SCRIP $FILE" | tee $AFTER_UPGRADE_SCRIPTS_HOME/install.log
                ./$FILE
                echo "THE SCRIPT $FILE WAS RUN" | tee $AFTER_UPGRADE_SCRIPTS_HOME/install.log
