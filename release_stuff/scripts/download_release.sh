...
then
    ...
    if [ -n "$OPEN_JDK_V17_URL" ]
    then
        # Primeiro verificar se ja temos essa versao instalada
        echo "Download the JDK version 17"

        FILE_NAME=$(echo "$OPEN_JDK_V17_URL" | rev | cut -d'/' -f 1 | rev)
        echo "FILE_NAME = $OPEN_JDK_V17_URL"

        echo "Downloading $FILE_NAME"
        wget --continue --retry-connrefused --tries=$MAX_TRIES --timeout=$TIMEOUT_SECONDS --wait=$WAIT_SECONDS "$OPEN_JDK_V17_URL"
        if [ $? -ne 0 ]; then
            echo "Failed to download $OPEN_JDK_V17_URL"            
        fi
