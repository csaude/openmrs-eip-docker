...
$SCRIPTS_DIR/download_release.sh "$RELEASES_PACKAGES_DIR" "$RELEASE_NAME" "$OPENMRS_EIP_APP_RELEASE_URL" "$EPTS_ETL_API_RELEASE_URL" "$CENTRALIZATION_FEATURES_MANAGER_RELEASE_URL" "$OPEN_JDK_V17_URL"
...
OPEN_JDK_V17_FILE_NAME=$(getFileName "$OPEN_JDK_V17_URL")
...
mkdir -p "$HOME_DIR/jdk/" && tar -xzvf "$CURRENT_RELEASES_PACKAGES_DIR/$OPEN_JDK_V17_FILE_NAME" -C "$HOME_DIR/jdk/"
