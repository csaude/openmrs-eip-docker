...
$HOME_DIR/scripts/execute_script_on_db.sh $DB_HOST $DB_HOST_PORT $DB_USER $DB_PASSWD $DB_NAME $UPDATE_DATE_SENT_SCRIPT 

echo "Executing script to try to update event date where it has status equal to SENT and event_date is NULL"

$HOME_DIR/scripts/execute_script_on_db.sh $DB_HOST $DB_HOST_PORT $DB_USER $DB_PASSWD $DB_NAME $UPDATE_EVENT_DATE_SCRIPT
